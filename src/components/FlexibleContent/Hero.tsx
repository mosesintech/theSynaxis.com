...
const Hero: React.FC<HeroProps> = props => {
  const { heroTitle, heroSubtitle, heroText, heroButton, heroSecondaryButton } = props;
  return (
    <>
      <Edges size="lg">
            ...
            <main className="mt-16 mx-auto max-w-7xl px-4 sm:mt-24">
              <div className="text-center">
                  <h1 className="text-4xl tracking-tight font-extrabold font-headingStyled text-gray-900 sm:text-5xl md:text-6xl">
                    {heroTitle && (
                      <span className="block xl:inline">{heroTitle}</span>
                    )}
                    {heroSubtitle && (
                      <span className="block text-indigo-600 xl:inline">{heroSubtitle}</span>
                    )}
                  </h1>
                {heroText && (
                  <p 
